...
if [ "$MVN_ARG_LINE" != "" ] ; then
    ...
    "$mvnBin" -v
    echo
    projects=( "*-model" "*-kjar" "portable-generator-service-service")
...
if [[ "$@" =~ "docker" ]]; then
    ...
    docker run -d -p 8090:8090 --name portable-generator-service-service apps/portable-generator-service-service:1.0-SNAPSHOT
elif [[ "$@" =~ "openshift" ]]; then
    echo "Launching the application on OpenShift..."
    
    oc new-app portable-generator-service-service:1.0-SNAPSHOT
    oc expose svc/portable-generator-service-service
else

	echo "Launching the application locally..."
	pattern="portable-generator-service-service"
	files=( $pattern )
	cd ${files[0]}
