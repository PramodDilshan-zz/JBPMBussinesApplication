...
if [ "$MVN_ARG_LINE" != "" ] ; then
    ...
    "$mvnBin" -v
    echo
    projects=( "*-model" "*-kjar" "PortableGeneratorService-service")
...
if [[ "$@" =~ "docker" ]]; then
    ...
    docker run -d -p 8090:8090 --name PortableGeneratorService-service apps/PortableGeneratorService-service:1.0-SNAPSHOT
elif [[ "$@" =~ "openshift" ]]; then
    echo "Launching the application on OpenShift..."
    
    oc new-app PortableGeneratorService-service:1.0-SNAPSHOT
    oc expose svc/PortableGeneratorService-service
else

	echo "Launching the application locally..."
	pattern="PortableGeneratorService-service"
	files=( $pattern )
	cd ${files[0]}
