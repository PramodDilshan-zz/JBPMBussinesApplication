...
if "%*" == "" (
	...
	for %%s in ("-model" "-kjar" "PortableGeneratorService-service") do (
...
:startapp
	if not x%mavenInput:docker=%==x%mavenInput% (
		...
		call docker run -d -p 8090:8090 --name PortableGeneratorService-service apps/PortableGeneratorService-service:1.0-SNAPSHOT
	) else if not x%mavenInput:openshift=%==x%mavenInput% (
		echo Launching the application on OpenShift...
		call oc new-app PortableGeneratorService-service:1.0-SNAPSHOT
		call oc expose svc/PortableGeneratorService-service
	) else (
		echo "Launching the application locally..."
		setlocal EnableDelayedExpansion
		cd PortableGeneratorService-service
		cd target
